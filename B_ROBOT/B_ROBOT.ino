...
// 		The output of the control (motor speed) is integrated so its really an acceleration
...
float dmpGetPhi() {
	...
	return (atan2(2 * (q.y * q.z + q.w * q.x),
			q.w * q.w - q.x * q.x - q.y * q.y + q.z * q.z) * RAD2GRAD);
}
...
float stabilityPDControl(float DT, float input, float setPoint, float Kp,
	...
	output = Kp * error
			+ (Kd * (setPoint - setPointOld) - Kd * (input - PID_errorOld2))
					/ DT;       // + error - PID_error_Old2
...
// Dividimos en 8 subperiodos para aumentar la resolucion a velocidades altas (periodos pequeos)
...
//	debug_counter++;
	...
	if (fifoCount >= 18) {
		...
		} else   // Robot not ready, angle > 70deg
		{
			setMotorSpeed(0, 0);
			setMotorSpeed(1, 0);
